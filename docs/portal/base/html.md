...
:::tip 提示
HTML 实体的更多信息请见 [w3cschool](https://www.w3school.com.cn/html/html_entities.asp)
:::
...
- `_self`  
  ...
  在当前窗口中打开，默认就是 _self。当然，`target` 属性也可以设置为内联框架中打开，其值就是内联框架的 `name` 属性值。也可以使用 `a` 标签来发送邮件，它会默认打开计算机的邮件客户端。
