...
char[] a8 = new char[3];   // 元素默认全为 0 或 '\u000', 而不是 '0'
...
如果 "hello" 在常量池不存在，则会在常量池里面创建一个，然后再创建 String 对象并指向常量池中的对象，此处产生了 2 个对象；如果常量池中存在 "hello"，则只会在堆中创建 String 对象并执行常量池中的对象。
:::
